...
export function payloadId(): number {
  const datePart: number = new Date().getTime() * 10 ** 3;
  const extraPart: number = Math.floor(Math.random() * 10 ** 3);
...
export function getChainData(chainId: number): IChainData {
  ...
  const API_KEY = process.env.NEXT_PUBLIC_ALCHMEY_API_KEY;

  if (!API_KEY) {
    throw new Error('Environment variable NEXT_PUBLIC_ALCHMEY_API_KEY is not set');
  }
